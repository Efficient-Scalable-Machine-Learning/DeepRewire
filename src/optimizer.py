...
class DEEPR(Optimizer):
	def __init__(self, params, nc=required, lr=required, l1=0.0, temp=None):
		...
		defaults = dict(lr=lr, l1=l1, temp=temp)
		...
		if self.nc > self.n_parameters:
			raise ValueError(f"Number of connections can't be bigger than number of parameters: nc:{nc} np:{self.n_parameters}")
	...
	def attempt_activation(self, candidate_idx):
			"""
			function will activate a connection if previously inactive and return 1. If was already active return 0.
			"""
			for group in self.param_groups:
				for p in group['params']:
					if p.grad is None:
	...
	def step(self, closure=None):
		...
		# look how many connections are inactive and activate if necessary. This is done by randomly sampling any index and trying.
		# If we assume a sparse network this should work better than saving the active or inactive indicies beforehand. 
