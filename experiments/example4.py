...
"""
In this experiment we will try to use a simple CNN and try it on CIFAR10.
We let it run only for 100 epochs.
"""
...
def plot(losses, losses2, accuracies, accuracies2, final_loss=None, final_accuracy=None, init_sparsity=None, final_sparsity=None, final_sparsity2=None):
        ...
        plt.savefig('example4.png', bbox_inches='tight', dpi=200)
        plt.close()
...
if __name__ == '__main__':
        ...
        for epoch in range(100):
                ...
                plot(np.array(losses), np.array(losses2), np.array(accuracies), np.array(accuracies2), init_sparsity=init_sparsity)
        ...
        pred = model(X)
        loss = criterium(pred, y).detach().cpu().numpy()
        accuracy = 0.0
        for _, (Xv, yv) in enumerate(val_dataloader):
                Xv, yv = Xv.to(device), yv.to(device)
                with torch.no_grad():
        ...
        plot(np.array(losses), np.array(losses2), np.array(accuracies), np.array(accuracies2), loss, accuracy, init_sparsity, final_sparsity, final_sparsity2)
