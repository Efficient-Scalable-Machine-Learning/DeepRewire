...
def plot(losses, losses2, accuracies, accuracies2, final_loss=None, final_accuracy=None, init_sparsity=None, final_sparsity=None, final_sparsity2=None):
	...
	plt.savefig('example6.png', bbox_inches='tight', dpi=200)
	plt.close()
...
if __name__ == '__main__':
	...
	for epoch in range(100):
		...
		pb.prnt()
		pb.increment()
		mapping_dict = {k: v for k, v in enumerate(random.sample(range(10), 10))}

		avg_loss = 0
		avg_loss2 = 0
		for (X, y) in train_dataloader:

			y = y.clone().apply_(lambda x: mapping_dict[x])
		...
		for _, (Xv, yv) in enumerate(val_dataloader):
			with torch.no_grad():
				...
				yv = yv.clone().apply_(lambda x: mapping_dict[x])
				# SoftDEEPR
				pred = model(Xv)
